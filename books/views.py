...
class BookDetailView(FormMixin, DetailView):
    ...
    def form_valid(self, form):
        ...
        new_comment.save()
        messages.success(self.request, "Ваш комментарий добавлен, спасибо")
...
def return_book_view(request, slug):
    try:
        ...
        if b:
            ...
            log_history.delete()
            messages.success(
                request, f'Вы успешно вернули книгу: {b.title}')
        else:
            messages.warning(
                request, f'Произошла ошибка, извините')
            return redirect('UserProfile')
    except Book.DoesNotExist:
        raise Http404("Книга недоступна сейчас ")
    return redirect('UserProfile')
...
def rate_book_view(request, slug, rating):
    try:
        ...
        raise Http404("Книга недоступна")
    return redirect('bookDetail', slug=b.slug)
...
def contact_form(request):
    ...
    if request.user.is_authenticated:
        ...
        form.fields['message'].widget.attrs['placeholder'] = 'Напишите ваше сообщение здесь'
